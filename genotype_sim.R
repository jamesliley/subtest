...
pi2= sample(c(0.1,0.01,0.001),1) # Randomly choose this
pi1= sample(c(0.2,0.1,0.05),1) # assume at least half of all SNPs are null
pi0=1-pi1-pi2
    
q2SEa1=sample(c(1.2,1.5,2),1) # 95% quantile for odds ratios between cases/controls for SNPs in group 2
q2SEd= sample(c(1,1.2,1.5,2),1) # Same for odds ratios between subtypes for SNPs in group 3

if (!null_model) {
  q2SEa2=sample(c(1.2,1.5,2),1) # 95% quantile for odds ratios between cases/controls for SNPs in group 3 (only chosen if null_model=FALSE)
