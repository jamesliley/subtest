...
require(mnormt)
  ...
  lhood <- function(pars, sumlog=TRUE) {
    ...
    if(sumlog) {
      sum(weights*log(e)) + (C*log(pars[1]*pars[2]*(1-pars[1]-pars[2]))) 
    } else e
  }
  ...
  while(df>tol & nit<maxit) {
    ...
    if ((nit %% n_save)==0) { # Print or save 'pars' every n iterations
      if (verbose) print(c(nit,pars,lhood(pars)),digits=6)
      if (!is.null(file)) {
        hist=value[1:nit,]
    ...
    if (!fit_null) {
      ...
      s1=pars[3]; s2=pars[5]; lower=0; upper=s1*s2-0.001; # Fit rho
...
##' @param y object of class 3Gfit; generally output from fit.3g
##' @author James Liley
