...
vec=c(lh0,lh1,la1-la0,n0x,n1x,seed,pars1,pars0)
...
##' @returns two element vector; gamma, kappa.
##' @examples
...
cut1=quantile(S$plr,0.9) # below this cutoff, use empirical quantile in PLR
...
##'  plot((1:length(pp))/(1+length(pp)),sort(pp),xlab="Quantile in U(0,1)",ylab="P-value"); abline(0,1,col="red")
##' p_mixchi(10,gamma,kappa)
p_mixchi=function(x,S=NULL,gamma=NULL, kappa=NULL) {
...
(kappa*pchisq(x/gamma,df=1,lower.tail=FALSE)) + ((1-kappa)*pchisq(x/gamma,df=2,lower.tail=FALSE))
